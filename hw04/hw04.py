...
def balanced(m):
    ...
    if is_planet(m) == True:
        return True
    leftmob = end(left(m))
    rightmob = end(right(m))
    leftlen = length(left(m))
    rightlen = length(right(m))
    if total_mass(rightmob) * rightlen == total_mass(leftmob) * leftlen and balanced(leftmob) and balanced(rightmob):
        return True
    return False
...
def max_path_sum(t):
    ...
    for i in range(len(branches(t))):
        maxval = max(maxval, label(t) + max_path_sum(branches(t)[i]))
    return maxval
