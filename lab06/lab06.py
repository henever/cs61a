...
def make_change(amount, coins):
    ...
    if amount < smallest:
        ...
        ans += [smallest]
        after_change_small = make_change(amount - smallest, rest) 
        if after_change_small == None:
            after_change_notsmall = make_change(amount, rest)
            if after_change_notsmall == None:
                return None
            else:
                return after_change_notsmall
        else:
            return ans + after_change_small
...
class ChangeMachine:
    ...
    def change(self, coin):
        if coin in self.coins:
            value = self.coins.pop(coin) +1
            self.coins[coin] = value
        else:
            self.coins[coin] = 1 # check whether 1 is in coins

        temp = make_change(coin, self.coins)
        for i in temp:
            value = self.coins.pop(i) - 1
            if value:
                self.coins[i] = value
        return temp
