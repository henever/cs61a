...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, entrance_num = 0):
        ...
        while place_now.is_hive == False : 
            if place_now.bees == [] or entrance_num < self.lower_bound:
                if entrance_num >= self.upper_bound:
                    return None
...
class LongThrower(ThrowerAnt):
    ...
    def __init__(self, health=1, place=None):
        super().__init__(health, place)  # Pass both arguments to the base class
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        place_fireant = self.place
        if place_fireant is not None:
            for bees_reduce in list(place_fireant.bees):
                bees_reduce.reduce_health(amount)
        if self.health > amount:
            self.lifeflag = True

        super().reduce_health(amount)
        if not self.lifeflag and place_fireant is not None:
            for bees_reduce in list(place_fireant.bees):
                bees_reduce.reduce_health(self.damage)
        # END Problem 5  
# BEGIN Problem 6
# The WallAnt class
class WallAnt(Ant):
...
class TankAnt(ContainerAnt):
    ...
    def do_damage(self, damage):
        if self.place.bees:
            for target_bee in list(self.place.bees):
                target_bee.reduce_health(damage)
    
    def action(self, gamestate):
        self.do_damage(self.damage)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place_now:
            if place_now.ant:
                if place_now.ant.is_container:
                    if place_now.ant.ant_contained:
                        place_now.ant.ant_contained.double()
                place_now.ant.double()
