...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def says(score0, score1):
        nonlocal last_score, running_high
        pre_score = last_score
        highest_score = running_high
        ##print(score0, score1)
        if who == 0 :
            if score0 - pre_score > highest_score :
                highest_score = score0 - pre_score
                print(highest_score, "point(s)! The most yet for Player", who)
            pre_score = score0
        if who == 1 :
            if score1 - pre_score > highest_score :
                highest_score = score1 - pre_score
                print(highest_score, "point(s)! The most yet for Player", who)
            pre_score = score1
        return announce_highest(who, pre_score, highest_score)
    return says
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    least_roll = 1
    max_pt = 0
    average_pt = make_averaged(roll_dice, trials_count)
    while roll_nums <= 10 :
        
        ave = average_pt(roll_nums, dice)
        if ave > max_pt :
            max_pt = ave
            least_roll = roll_nums
        roll_nums += 1
    return least_roll
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if extra_turn(score, opponent_score) == True :
        ...
        return 0
    elif extra_turn(score+free_bacon(opponent_score), opponent_score) == True :
        return 0
    else :
        return num_rolls
